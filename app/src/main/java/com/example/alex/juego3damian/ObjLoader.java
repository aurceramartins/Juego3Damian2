...
public class ObjLoader {
    public static Vertices3 load(GLGame game, String file) {
        ...
        try {
            ...
            short[] facesVerts = new short[lines.size() * 3];
            short[] facesNormals = new short[lines.size() * 3];
            short[] facesUV = new short[lines.size() * 3];
            ...
            for (int i = 0, vi = 0; i < numFaces * 3; i++) {
                int vertexIdx = (int) (facesVerts[i] * 3);
                verts[vi++] = vertices[vertexIdx];
                verts[vi++] = vertices[vertexIdx + 1];
                verts[vi++] = vertices[vertexIdx + 2];

                if (numUV > 0) {
                    int uvIdx = (int) (facesUV[i] * 2);
                    verts[vi++] = uv[uvIdx];
                    verts[vi++] = 1 - uv[uvIdx];

                }
                if (numNormals > 0) {
                    int normalIdx = (int) (facesNormals[i] * 3);
            ...
            throw new RuntimeException("No se puede cargar el fichero'" + file + "'", e);
        } finally {
            if (in != null) {
                try {
                    in.close();
